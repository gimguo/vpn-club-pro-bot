...
Начиная использование сервиса, Пользователь подтверждает, что ознакомился с настоящим Пользовательским соглашением и Политикой конфиденциальности до начала использования сервиса."""
...
Оператор взаимодействует с Роскомнадзором в установленном законом порядке. Оператор использует автоматизированные средства для ограничения доступа к ресурсам, включённым в Единый реестр запрещённой информации, в соответствии с требованиями Роскомнадзора.
...
• Приглашающий получает бонусные дни за каждого нового пользователя
...
• При выявлении нарушений Оператор вправе:
  — заблокировать аккаунт пользователя;
  — аннулировать начисленные бонусы;
  — прекратить предоставление услуг без возмещения стоимости подписки"""
...
• Возврат средств возможен в течение 24 часов, если услуга не использовалась (объём переданных данных = 0)
• Возврат осуществляется на тот же платёжный инструмент, с которого была произведена оплата
• Оператор вправе отказать в возврате при обнаружении злоупотреблений (многократное использование пробного периода, мошенничество)
...
• Оператор не несёт ответственности за косвенные убытки, упущенную выгоду или иной ущерб, возникший в результате использования или невозможности использования сервиса
...
Согласие может быть отозвано в любой момент через «💬 Поддержка» (ст. 9 ч. 2 ФЗ-152).
Подробности — /privacy

<b>8. Изменение условий</b>
• Оператор вправе изменять условия с предварительным уведомлением через бот не менее чем за 10 календарных дней до вступления изменений в силу
...
<b>7. Локализация, хранение и обезличение данных</b>
• Учётные данные пользователей (Telegram ID, подписки, платежи) хранятся в базе данных на территории Российской Федерации (п. 5 ст. 18 ФЗ-152)
• VPN-серверы расположены за пределами РФ, однако <b>не хранят</b> персональные данные пользователей
• При обработке данных в аналитических и статистических целях применяется обезличение методом хеширования идентификаторов (SHA-256). Обезличенные данные используются исключительно для агрегированной статистики сервиса и не позволяют идентифицировать конкретного пользователя
...
• Данные об оплатах (ID транзакций) — 5 лет с даты последней операции по подписке (бухгалтерский учёт)
• По запросу пользователя данные удаляются в течение 30 дней. После удаления Оператор направляет подтверждение факта уничтожения в течение 5 рабочих дней
• Оператор формирует акт об уничтожении персональных данных и хранит его в течение 3 лет (ст. 21 ФЗ-152)"""
...
• Доступ к персональным данным имеют только уполномоченные лица Оператора, прошедшие инструктаж по информационной безопасности
• Резервное копирование данных с шифрованием
• Регулярное обновление программного обеспечения
• Периодические тесты на проникновение (pentest) для выявления уязвимостей
• Серверы в защищённых дата-центрах с физическим контролем доступа

<b>11. Реагирование на инциденты</b>
При обнаружении утечки персональных данных Оператор обязуется:
• Приостановить обработку затронутых данных до устранения причин инцидента
...
<b>12. Передача данных третьим лицам</b>
Данные не передаются третьим лицам, за исключением:
• <b>ЮKassa</b> — получает только ID транзакции и сумму платежа. Персональные данные пользователя (Telegram ID, имя) в ЮKassa не передаются. ЮKassa обрабатывает платежи в соответствии с ФЗ-152 и ФЗ № 161-ФЗ «О национальной платёжной системе»
• <b>Роскомнадзор</b> — по требованию в случаях, предусмотренных законом
• <b>Иные уполномоченные органы</b> — исключительно на основании судебного решения
