...
async def show_terms(message: Message):
    ...
    text = """📜 <b>Условия использования</b>

<b>1. Описание сервиса</b>
VPN Club Pro предоставляет услугу защищённого доступа к сети Интернет посредством шифрованного прокси-канала. Сервис предназначен для обеспечения конфиденциальности данных, защиты соединения в публичных сетях Wi-Fi и безопасного удалённого доступа.

<b>2. Пользователь обязуется</b>
• Использовать сервис в соответствии с законодательством своей страны
• Не использовать сервис для распространения вредоносного ПО, спама или иной противоправной деятельности
• Не передавать учётные данные (VPN-ключ) третьим лицам
...
• Запрещён массовый спам реферальными ссылками
• Администрация оставляет за собой право аннулировать бонусы, полученные с нарушением правил

<b>4. Оплата и возвраты</b>
• Пробный период — бесплатно, одноразово
• Платные подписки активируются после подтверждения оплаты
• Возврат средств возможен в течение 24 часов после оплаты, если услуга не использовалась
...
• Администрация не гарантирует 100% доступность сервиса и оставляет за собой право на проведение технических работ

<b>6. Конфиденциальность</b>
• Мы не храним логи сетевой активности пользователей
• Собираемые данные: Telegram ID, дата регистрации, тип подписки
• Данные не передаются третьим лицам

<b>7. Прочее</b>
• Администрация оставляет за собой право изменять условия с уведомлением пользователей
• Продолжая использование сервиса, вы соглашаетесь с актуальной версией условий

По вопросам: используйте меню «💬 Поддержка»
...
async def show_terms_callback(callback: CallbackQuery):
    """Условия использования через inline кнопку"""
    text = """📜 <b>Условия использования</b>

Используя VPN Club Pro, вы соглашаетесь:
• Использовать сервис в соответствии с законами вашей страны
• Не передавать VPN-ключ третьим лицам
• Не использовать сервис в противоправных целях

Полная версия: /terms

Сервис предназначен для защиты данных и конфиденциальности."""

    from aiogram.types import InlineKeyboardMarkup, InlineKeyboardButton
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📜 Полная версия", url=f"https://t.me/{settings.bot_username}?start=terms")],
