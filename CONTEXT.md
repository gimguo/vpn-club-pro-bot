...
- `/terms` — Пользовательское соглашение (ФЗ-149, запрет доступа к заблокированным ресурсам)
- `/privacy` — Политика конфиденциальности (ФЗ-152, обработка ПД)
- **Согласие при регистрации:** `/start` показывает экран принятия условий + обработки ПД
- Поля в User: `terms_accepted`, `terms_accepted_at`
- Сервис позиционируется как **защита данных и шифрование**, НЕ обход блокировок
